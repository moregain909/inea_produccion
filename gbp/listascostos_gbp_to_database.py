...
from gbp_helpers import ExcelListasCostosGbp
...
if __name__ == "__main__":
    ...
    listas_costos_gbp = ExcelListasCostosGbp()
    listas_costos_gbp.get_data(filename = "gbp_listascostos.xlsx")

    # Inserts into gbp_several_tables table the listas_precios the items that aren't present already
    for lista_gbp in listas_costos_gbp.items:
        lista_db = GbpSeveralTables_DbItem()
        lista_db.get_data_from_gbp_item(lista_gbp)
        #logging.debug(f'lista_db: {lista_db}')
