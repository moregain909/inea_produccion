...
from gbp_helpers import ExcelTiendasGbp
...
if __name__ == "__main__":
    ...
    tiendas_gbp = ExcelTiendasGbp()
    tiendas_gbp.get_data(filename = "gbp_tiendas.xlsx")

    # Inserts into gbp_several_tables table the listas_precios the items that aren't present already
    for tienda_gbp in tiendas_gbp.items:
        tienda_db = GbpSeveralTables_DbItem()
        tienda_db.get_data_from_gbp_item(tienda_gbp)
        #logging.debug(f'tienda_db: {tienda_db}')

        if not tienda_db.is_on_db(session):
            logging.info(f'Insertando en la base de datos {tienda_db.gbp_table} | {tienda_db.gbp_id} | {tienda_db.name}')
            tienda_db.insert_into_db(session)
