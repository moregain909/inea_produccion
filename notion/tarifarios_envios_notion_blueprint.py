...
tarifarios_envios_notion_blueprint = Blueprint('tarifarios_envios_notion_blueprint', __name__, template_folder="templates")
...
@tarifarios_envios_notion_blueprint.route("/tarifarios", methods=['GET', 'POST'])
def tarifarios():

    if request.method == "POST":
        
        if "add" in request.form:
            if not request.form["col_tarifario"] or not request.form["col_rollup"]:
                flash(f"No se agregó el tarifario! Se requiere ingresar Columna Tarifario y Columna Rollup.")
            else:
                nueva_col_tarifario = request.form["col_tarifario"]
                nueva_col_rollup = request.form["col_rollup"]
                tarifarios_nuevos.append((nueva_col_tarifario, nueva_col_rollup))

                return render_template("tarifarios.html", tarifarios_nuevos=tarifarios_nuevos)
        
        elif "process" in request.form:
            error_msg = ""
            if (not request.form["col_tarifario"] or not request.form["col_rollup"]) and not tarifarios_nuevos:
                error_msg = error_msg + "Se requiere ingresar Columna Tarifario y Columna Rollup. "

            if not request.form["formula_original"]:
                error_msg = error_msg + "Se requiere ingresar la fórmula original. "
                
            if error_msg != "":
                flash(f"No se procesó el nuevo tarifario! {error_msg}")
                return render_template("tarifarios.html")
            
            vieja_formula = request.form["formula_original"]
            nueva_fórmula = genera_formula_nueva(vieja_formula, tarifarios_nuevos)
            
            flash(f"Nueva fórmula generada!")
            print(f'Fórmula nueva: {nueva_fórmula}')
            return render_template("tarifarios.html", nueva_formula=nueva_fórmula, vieja_formula=vieja_formula)
            
    return render_template("tarifarios.html", tarifarios_nuevos=tarifarios_nuevos)
    
    #return render_template("tarifarios.html", nueva_fórmula=nueva_fórmula)


if __name__ == "__main__":
    nueva_fórmula = genera_formula_nueva(FORMULA_ORIGINAL, TARIFARIOS_NUEVOS)
    pass